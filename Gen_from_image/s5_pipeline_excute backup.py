...
logger.info(f"[LOGGING] Bắt đầu ghi log vào {log_file.name}\n")
...
def excute_sequence_files(args: Args):
    ...
    for row in args.csv.itertuples():
        # logger.info(row.Index, row.template_name, row.size_x)
        ...
        logger.info(f"alter name ------ {_args.alter_item_name}")
        ...
        logger.info(f"row style {row.style}")
        logger.info(f"row style != '' ? {row.style != ''}")
