...
class Analyzer:
    ...
    def analyze_level(self, level_data, filename):
    ...
    def excute(self, args):
        os.makedirs(os.path.dirname(args.analyze_output_csv_path), exist_ok=True)
        files = [f for f in os.listdir(args.solver_solved_data) if f.endswith(".json") or f.endswith(".json.gz")]
        results = []
        ...
        for file in files:
            ...
            try:
                ...
                info = self.analyze_level(data, file)
                results.append(info)
        ...
        # Write CSV
        with open(args.analyze_output_csv_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([
                "Level Name",
                "Total Arrows",
        ...
        print(f"\n✅ CSV saved to: {args.analyze_output_csv_path}")

if __name__ == "__main__":
    
    args = Args()
    args.solver_solved_data = "_solved_data"
    args.analyze_output_csv_path = "out_put/analysis.csv"

    analyzer = Analyzer()
    analyzer.excute(args)
