...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QHBoxLayout,
                               ...
                               QDoubleSpinBox, QComboBox, QDockWidget, QScrollArea, QFrame)

# --- CORE LOGIC IMPORTS ---
try:
    from core.validator import Validator
    from core.generator import HybridLevelGeneratorTestable
    from core.helper import Arrow, Args
    # Import module xử lý ảnh mới
    from image_tools import ImageTools
except ImportError as e:
    print(f"CRITICAL ERROR: Could not import core modules. Make sure validator.py, generator.py, helper.py, image_tools.py are in the same folder.\nDetails: {e}")
    sys.exit(1)
...
# --- STYLE PRESETS (UPDATED NAMES) ---
STYLE_PRESETS = {
    "Aztec":      {"straight": 1.5, "left": 1.0, "right": 1.0, "max_turns": 5},  # Balanced
    "Snake":      {"straight": 0.5, "left": 2.0, "right": 2.0, "max_turns": 15}, # Winding
    "Spaghetti":  {"straight": 1.0, "left": 1.0, "right": 1.0, "max_turns": 20}, # Chaos
    "Linear":     {"straight": 5.0, "left": 0.5, "right": 0.5, "max_turns": 2},  # Straight
...
class AnimatedArrow:
    """Class dùng riêng cho UI để hiển thị animation playtest"""
...
class CanvasWidget(QWidget):
    ...
    def __init__(self, status_bar, parent=None):
        ...
        # Playtest specific variables
        self.playtest_arrows = []
        self.animating_arrows = []
        self.movable_arrows_cache = [] # Cache danh sách mũi tên có thể đi
        self.hovered_arrow = None      # Mũi tên đang được trỏ vào (trong mode Playtest)
        
        # Validator được gọi từ module ngoài
    ...
    def get_all_arrows(self):
        """Trả về tất cả mũi tên từ TẤT CẢ các layer"""
    ...
    def refresh_playtest_logic(self):
        """Tính toán lại các mũi tên có thể di chuyển (Valid moves)"""
    ...
    def paintEvent(self, event):
        ...
        if self.mode != "PLAYTEST":
            # Draw Editable Areas (All layers)
            if self.show_areas:
            ...
            for arrow in self.playtest_arrows:
                # Check nếu arrow này đang được hover -> Highlight lên
    ...
    def draw_arrow(self, painter, points, direction, color, is_highlighted=False):
        ...
        if is_highlighted:
            draw_color = draw_color.lighter(150) # Làm sáng màu lên
            thickness *= 1.2 # Làm dày hơn chút
    ...
    def mouseMoveEvent(self, event):
        ...
        # --- PLAYTEST HOVER LOGIC ---
        if self.mode == "PLAYTEST":
            # Kiểm tra xem chuột có đang nằm trên mũi tên movable nào không
            prev_hover = self.hovered_arrow
            self.hovered_arrow = None
            
            # Chỉ check trong danh sách movable (được cache)
            ...
            # Chỉ update lại màn hình nếu trạng thái hover thay đổi
            if prev_hover != self.hovered_arrow:
                self.update()
            return
        ...
        if self.mode == "PAINT_AREA" and self.active_layer:
            ...
            # Logic vẽ mũi tên thủ công (chỉ cho phép đi thẳng/vuông góc)
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.mode == "MANUAL_ARROW" and self.active_layer and len(self.temp_arrow_path) > 1:
            ...
            if not self.get_all_occupied_points().isdisjoint(set(path_to_save)):
                ...
                # Dùng Arrow class từ helper.py
                hypo_arrow = Arrow(path_to_save, direction,
                                   self.active_layer.id, -1,
                                   self.active_layer.color)
                self.validator.clear_cache()
                # === [FIX LOGIC HERE] ===
                # Cần truyền TOÀN BỘ mũi tên (của tất cả layer) vào để kiểm tra
...
# --- MAIN WINDOW (REFACTORED WITH DOCK WIDGETS) ---

class MainWindow(QMainWindow):

    def __init__(self):
        super().__init__()
        self.setWindowTitle("Level Editor - Pro Layout v3.0")
    ...
    def load_level(self):
        ...
        try:
            ...
            for i, arrow_data in enumerate(data.get("Arrows", [])):
                ...
                # Create Arrow using Imported Class
                arrow = Arrow(points, (dx, dy), layer.id, i, layer_color)
                layer.arrows.append(arrow)
                layer.editable_area.update(points) # Auto-add to editable area
    ...
    def start_playtest(self):
        ...
        # Calculate bounds to frame the view
        ...
        # Initial calculation for Playtest
    ...
    def process_image_import(self):
        """
        Quy trình xử lý ảnh -> board:
        1. Chọn file ảnh.
        2. Resize & Convert (dùng ImageTools).
        3. Cập nhật vào Layer hiện tại.
        """
        ...
        # 3. Call ImageTools
        result = self.image_tools.process_image_to_points(
        ...
        if result["success"]:
            # 4. Update Board
            # Clear old board first? Tùy nhu cầu. Ở đây tôi sẽ Clear layer hiện tại để load ảnh mới.
            self.canvas.active_layer.arrows.clear()
            self.canvas.active_layer.editable_area = result["points"]
            
            # Cập nhật kích thước canvas cho khớp
    ...
    def generate_level_smart(self):
        """
        Đây là hàm tái hiện lại logic của 's3_gen1_cli_generator.py'.
        Nếu chọn 'Smart Fill', nó sẽ chạy vòng lặp giảm dần độ dài mũi tên
        để lấp đầy board.
        """
        ...
        # 1. Setup Arguments (Mocking Args object)
        current_len = self.gen_avg_len_spinbox.value()
        min_len = 3 # Hardcode hoặc lấy từ UI nếu cần
        fake_args = Args(
            alter_item_name=f"Manual_L{active_layer.id}",
            min_width=self.gen_min_width_spinbox.value(),
            max_width=self.gen_max_width_spinbox.value()
        )

        # 2. Setup Generator & Validator
        validator = Validator()
        generator = HybridLevelGeneratorTestable(fake_args)
        
        # 3. Preparation
        start_arrow_id = self.canvas.arrow_id_counter
        all_arrows = self.canvas.get_all_arrows() # Các mũi tên hiện có (coi như chướng ngại vật)
        
        total_cells_to_fill = len(active_layer.editable_area)
        
        is_loop = self.chk_smart_fill.isChecked()
        max_limit = self.gen_arrow_count_spinbox.value() # Hard limit của user
        
        generated_count = 0
        
        # 4. GENERATION LOOP (The "Smart" Part)
        while current_len >= min_len:
            # a. Tính toán không gian còn lại
            ...
            # b. Tính số lượng cần tạo cho lượt này
            num_to_gen = self.suggest_arrow_count(len(available), current_len)
            
            # Giới hạn bởi max user input (nếu user chỉ muốn thêm tối đa 5 con chẳng hạn)
            remaining_quota = max_limit - generated_count
            if num_to_gen > remaining_quota and not is_loop:
                num_to_gen = remaining_quota
            ...
            # c. Gọi Generator Core
            # Lưu ý: Generator tự xử lý việc chuyển tọa độ sang Virtual Board
            if self.chk_advanced_mode.isChecked():
                new_arrows, new_id_counter, msg = generator.generate_hybrid_level_advanced(
                    validator=validator,
                    active_layer=active_layer,
                    all_arrows_on_board=all_arrows,
                    start_arrow_id=start_arrow_id,
                    num_to_gen=num_to_gen,
                    avg_length=current_len,
                    straight_weight=self.spin_straight.value(),
                    left_weight=self.spin_left.value(),
                    right_weight=self.spin_right.value(),
                    max_turns=self.spin_max_turns.value()
                )
            else:
            ...
            # d. Cập nhật trạng thái (CÓ KIỂM TRA LẠI GLOBAL)
            if new_arrows:
                # --- GLOBAL VALIDATION FIX ---
                # Generator có thể đã bỏ qua các chướng ngại vật ở layer khác do việc cắt vùng (cropping).
                # Chúng ta cần kiểm tra lại xem việc thêm các mũi tên này vào bàn cờ TOÀN CỤC có hợp lệ không.
                self.canvas.validator.clear_cache()
                
                # Tạo một bàn cờ giả định gồm: Tất cả mũi tên cũ + Mũi tên mới sinh
                ...
                if is_globally_solvable:
                    ...
                    active_layer.arrows.extend(new_arrows) # Thêm vào UI layer
                    start_arrow_id = new_id_counter
                    generated_count += len(new_arrows)
                    self.canvas.update()
                    
                    # Logic của CLI: Nếu tìm thấy, giảm length để điền vào các lỗ nhỏ hơn
                    if is_loop and current_len > min_len:
                        current_len -= 2 # Step down
                        if current_len < min_len: current_len = min_len
                else:
                    # Nếu KHÔNG OK -> Từ chối lô này và thử giảm độ dài (hoặc dừng)
                    ...
                    # Không thêm vào all_arrows, buộc vòng lặp phải thử phương án khác (giảm length)
                    if current_len == min_len:
                        break
                    current_len -= 2
                    if current_len < min_len: current_len = min_len
            else:
                # Nếu không tìm thấy ở độ dài này, buộc phải giảm độ dài
                if current_len == min_len:
                    break # Đã bó tay ở độ dài nhỏ nhất
                current_len -= 2
                if current_len < min_len: current_len = min_len
            
            if not is_loop:
                break # Nếu không chọn loop, chỉ chạy 1 pass duy nhất
